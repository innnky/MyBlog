<template>
  <div class="main-container" >
    ...
    <!--      </section>-->
    <h3 class="title" style="text-align: left;">计划</h3>
    <div style="width: 100%;">
      <el-row class="content" type="flex" justify="center" >
        ...
        <el-col :md="18" :sm="18" :xs="22"
          ...
          <el-button @click.native="test(tableData)">aaaaa</el-button>
        </el-col>
      </el-row>
      <common-footer style="margin-top: 30px"/></div>
...
import {getAllPlans, updateStatus} from "@/api/data";
    ...
    test(rows){
      rows.forEach(row => {
        ...
        if (row.children.length > 0){
          this.test(row.children)
        }
      });
    },
    handleSelectionChange(stat){
      if (this.flag ===1){
        var res = "";
        stat.forEach((r)=>{
          res += r.id+",";
